...
from cadkit.joinery import PrintSpec, slide_joint, dovetail_dims
from .separator import _ratchet_star
from .frame import (
    _lever_mount, _beam, _arm_trim, _beam_bottom_trim, _lever_pin_bores,
    _box, _XC, _SB_Y0, _X_SPLIT,
)
from .params import (
    RIM_SEAT_Z, RATCHET_H, CONE_H, BRAKE_H, RIM_OD, RATCHET_DEPTH,
...
_T      = 1.6                         # stand-in wall thickness
...
_Z_TOP  = RIM_SEAT_Z + RATCHET_H + CONE_H + BRAKE_H   # 31.5 — band top
...
def _rim_wall():
    ...
    z2 = z1 + CONE_H
    prof = [(_R_ROOT, _BED), (_R_ROOT, z1), (_R_TIP, z2), (_R_TIP, _Z_TOP),
            (_R_TIP - _T, _Z_TOP), (_R_TIP - _T, z2), (_R_ROOT - _T, z1),
            (_R_ROOT - _T, _BED)]
    return (cq.Workplane("XZ").polyline(prof).close()
            .revolve(2.0 * _SWEEP, (0, 0), (0, 1))
...
def _build_rig():
    ...
    ch = (slide_joint(JOINT_WIDTH, (BEAM_Z1 + 1.0) - (_BED - 1.0),
                      tenon=up, mortise=up,
...
def _build_rig_wall():
    ...
    up = PrintSpec(nozzle=0.8, material="PETG-GF", facing="up")
    ten = (slide_joint(JOINT_WIDTH, _Z_TOP - WALL_Z0, tenon=up, mortise=up,
                       install="z", depth=JOINT_DEPTH)
    ...
    # channel at install.
    nk, hd, du = dovetail_dims(JOINT_WIDTH, JOINT_DEPTH, 0.8, JOINT_CLR,
                               JOINT_BACK_CLR)
