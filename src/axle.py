...
the spring's small flange hole, so the axle splits and the halves meet
inside the spring). The strip slits are OPEN-ENDED slots (user's call —
the closed-window experiment is reversed: the spring's central strip is
FIXED and cannot thread a window, so the halves SLIDE ONTO the strip,
v2's design): the separator's slit runs open through its +z tenon tip,
axle_top's through its collar bottom (the mortise mouth is split).
...
the Ø13.4 flange hole is on TOP now and the Ø8.7 on the bottom):
  ...
  * TOP half — the MORTISE half: a plain Ø8.0 shaft (NO head — user:
    with the collar at the bottom a head left no installable direction
    through the bearing; the half goes in from BELOW, up through the
    spring into the 608, and the rotating stack's weight rides the
    BOTTOM bearing via the separator — the top shaft only locates
    radially), then a 45° cone to the Ø11.5 MORTISE COLLAR entering
    the spring through the top Ø13.4 hole, bore + open-bottom strip
    slit down to the datum plane. Prints FLIPPED (flat shaft-top face
    on the bed; the shaft→collar cone widens going up in print — 45°,
    support-free).
...
def _build_top():
    ...
    a plain Ø8.0 shaft through the bearing (NO head — user, post-swap:
    with the collar at the bottom, a head on top left no installable
    direction; this half goes in from BELOW, up through the spring and
    into the 608, the strip entering the open-bottom slit as it rises)
    → 45° cone (inside the top flange's Ø13.4 hole zone) → Ø11.5
    collar down to the datum plane, carrying the female bore."""
    ...
    # slit OPEN through the collar bottom; the closed top end sits 3.2
    # in from the spring's edge — a print floor in this half's flipped
    # print; the shaft is SOLID from there up through the bearing
...
def _build_axle_separator():
    ...
    POCKETED (user #880: save material): the disc inside the drum bore
    hollows to a SEP_POCKET_FLOOR_T plate; the central BOSS walls the
    bottom-bearing cavity, and the root cone runs CONTINUOUSLY from the
    boss top to the Ø8 tenon post (no flat ring — user: strength; the
    45° completes at the shoulder plane, just under the spring's Ø8.7
    hole, which the bare post alone may pass). Post to the top datum,
    open-top strip slit."""
    # TENON POST, tip at JOINT_Z1 = the top half's slit ceiling plane
    b = cyl(AXLE_PRINT_D, JOINT_Z1 - SEP_Z1, z=SEP_Z1)
    # 45° ROOT CONE — post→boss, continuous to the cylinder (user):
    # d_top IS the post Ø; widens toward the bed: self-supporting
    b = b.union(cone_solid(
        d_bottom=AXLE_PRINT_D + 2.0 * (SHOULDER_Z - SEP_Z1),
        d_top=AXLE_PRINT_D,
        h=SHOULDER_Z - SEP_Z1, z_base=SEP_Z1))
    ...
    b = b.cut(_cable_pass())
    # DISC POCKET (user #880): hollow the disc between the bearing BOSS
    # and the drum bore, leaving the SEP_POCKET_FLOOR_T bottom plate as
    # the boss↔rim tie; walls vertical, floor a plain print plate
    b = b.cut(
        cyl(2.0 * SEP_POCKET_R,
            (SEP_Z1 - (SEP_Z0 + SEP_POCKET_FLOOR_T)) + 1.0,
