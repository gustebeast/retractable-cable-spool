...
from .params import (
    ...
    BOSS_BORE_ID, FLOOR_OD, FLOOR_MIN_BOT_Z, STATIC_MOVE_CLR,
    NOZZLE,
)
...
# Wall ↔ beams: dovetail, installs along Z (see wall.py — same width/depth/clr).
_WALL_JOINT = slide_joint(JOINT_WIDTH, MORTISE_L, tenon=_UP, mortise=_UP,
                          install="z", depth=JOINT_DEPTH)
# frame_top ↔ frame_bottom: ROTATIONAL octagon-arc joints (the axle leaves
# rotation as the only free motion — see params). The arc generators are
# called directly; clearance matches the linear family's print-tested table.
_TOP_NOZ, _TOP_CLR = 0.8, 0.1                     # PETG-GF, per the joinery table

# Arc-joint geometry, DERIVED. The tenon ring sits just outside the wall
# channels' radial keep-out; angular extents come from the arm width.
_R_T = BEAM_IR + _WALL_JOINT.height + TOP_WALL_CLEAR + TOP_JOINT_W / 2.0
...
def _plus(z0, z1):
    """Plus-shaped spider: two crossed TOP_RIB_SIZE-wide bars spanning ±_R_OUT."""
    h = z1 - z0
    bar_x = (cq.Workplane("XY").workplane(offset=z0)
             .rect(2 * _R_OUT, TOP_RIB_SIZE).extrude(h))
    bar_y = (cq.Workplane("XY").workplane(offset=z0)
             .rect(TOP_RIB_SIZE, 2 * _R_OUT).extrude(h))
...
def _wall_mortise(angle_deg):
    ...
    JOINT_SEAT_CLR below the seated wall tenon; the wall rests on the stops."""
    return (_WALL_JOINT.mortise(drop=2.0)
            .translate((WALL_OR, 0, WALL_Z0 - JOINT_SEAT_CLR))
            .rotate((0, 0, 0), (0, 0, 1), angle_deg))


def _arc_tenon(site_deg):
    """Arc tenon on a beam top, seated CENTERED on its arm: sweeps
    ±_TEN_HALF_A about the site angle at radius _R_T."""
    return (octagon_tenon_arc(TOP_JOINT_W, _R_T, 2.0 * _TEN_HALF_A,
                              _TOP_NOZ, _TOP_CLR, 1.0)
...
def _arc_mortise(site_deg):
    ...
    return (octagon_mortise_arc(TOP_JOINT_W, _R_T, sweep,
                                _TOP_NOZ, _TOP_CLR, 2.0)
...
_R_FLOOR = FLOOR_OD / 2.0


def _x_climb(y_inner):
    """Climb-start x for a branch whose inner face is at |y| = y_inner."""
    x_edge = math.sqrt(_R_FLOOR ** 2 - y_inner ** 2)
    return x_edge + STATIC_MOVE_CLR + BOT_RIB_Z1 - FLOOR_MIN_BOT_Z


# ONE shared climb start for all three branches — the centre's (full floor
# radius, the latest/most conservative). The sides could start ~2 mm earlier
# by their own edge math, but a shared start makes every 45° face and foot
# edge LINE UP across the trident (staggered starts read as protruding stubs).
_XC = _x_climb(0.0)                              # ≈ 68.9 — where the TOP face bends
...
def _thrust_boss(y_face, grow, pz, bore_id, clip_x=None):
    ...
    # NO round teardrop bore behind the ring (user's call): the square
    # axle's diamond cutter is the only hole — the 45°-clocked flanks
    # self-support, and the old round counterbore just left a sub-1.6
    # shelf on the face. The ring's own ID ceiling is a ~1.6 mm bridge
    # over the diamond's top corner — printable as-is.
...
def _lever_mount():
    ...
    # BRAKE rest tab (the original stop-pin role): its +X face catches the handle's
    # outer-y corner at rest. GROWS out of the climb's inner y-face with a
    # 45° underside ramp rooted on that face, and a FLAT ROOF (user's call —
    # was a 45° trim chasing the climb plane, which read as a pointy top)
    # that stays under the climb's top plane over the whole tab (≥ 0.6).
    # FULLY DERIVED from two rules (a hardcoded ramp endpoint once flattened
    # the 45° to ~36° when the columns moved out, user-caught): the flat
    # roof rides 0.6 under the climb's 45° top plane (z = x − _XC +
    # BOT_RIB_Z1) at the tab's worst (−x) edge, and the ramp anchor drops
    # so the inner-end wall (ramp → roof) is exactly 1.6 (was 1.2,
    # user-caught). 1.6 thick; 4.8 proud (user's call — was 3.2, more
    # handle-corner catch).
    _stop_x = LEVER_PIVOT_X - LEVER_HANDLE_W / 2.0 - 0.1
    _tab_w  = 4.8                                # proud of the climb face
    _tab_x0 = _stop_x - 1.6
    _roof   = (_tab_x0 - _XC + BOT_RIB_Z1) - 0.6 # ≈ 5.2 — under the climb plane
    _anchor = _roof - 1.6 - _tab_w               # ≈ −1.2 — ramp root z on the face
    tab = _box(_tab_x0, _stop_x, -_SB_Y0, -_SB_Y0 + _tab_w, _anchor, _roof)
    # 45° underside ramp off the climb's y face — plane z = y + _SB_Y0 + _anchor
    tab = tab.cut(_yz_prism(
        [(-_SB_Y0 - 0.5, _anchor - 0.5),
         (-_SB_Y0 + _tab_w + 1.0, _anchor + _tab_w + 1.0),
         (-_SB_Y0 + _tab_w + 1.0, _anchor - 6.0),
         (-_SB_Y0 - 0.5, _anchor - 6.0)],
        _stop_x - 2.0, _stop_x + 0.5))
    m = m.union(tab)
    return m
...
def _build_frame_top():
    ft = _plus(TOP_RIB_Z0, TOP_RIB_Z1)
    for a in (0.0, 90.0, 180.0, 270.0):
