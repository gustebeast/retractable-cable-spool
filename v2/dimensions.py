...
NOZZLE         =   0.8   # THE print-width unit (user's call 2026-07-22;
                         # was 0.4). Everything printed derives from it —
                         # bead-multiple sizing policy, 2026-08-03: walls,
                         # ribs, lips, tiers are k*NOZZLE; hardware dims,
                         # cable dims and small fit CLEARANCES stay
                         # absolute (user's exemption)
...
FLANGE_LIP_T   =   4 * NOZZLE   # rim thickness: 1.7 mm of clean support material above
...
BEARING_INNER_CH = 0.3   # 608 inner-ring bore-edge chamfer (spec r_s min ≈ 0.3):
                         # the axle top's 45° cone SEAT is placed so this factory
                         # chamfer face-mates it and the race bottom lands exactly
                         # at the nominal seat plane (flipped-print lip redesign)
BEARING_LIP_H  =   2 * NOZZLE   # retention lip axial height (was 1.0 — quality-tier
                         # bump, user's call)
BEARING_LIP_ID = BEARING_OD + BEARING_CLR - 4 * NOZZLE
                         # 19.0 — retention lip ID: a full 1.6 radial rib off
                         # the pocket bore Ø22.2 (was 20.0/1.1). The lip laps
                         # the 608's seal annulus (~Ø19.4 inward) by ~0.2 — fine,
                         # user's call: the seal sits on the OUTER ring, which
                         # is exactly what the pocket holds, so there is no
...
AXLE_LIP_W     =   2 * NOZZLE   # was 1.0 — quality-tier bump (user's call);
                         # lip OD 11.15 still lands on the 608 inner
                         # ring's face annulus (Ø8..~12.1)
AXLE_LIP_OD    =   AXLE_PRINT_D + 2 * AXLE_LIP_W   # 11.15 mm
AXLE_LIP_H     =   2 * NOZZLE   # was 1.0 — quality-tier bump (user's call)
...
KEY_W      = 2 * NOZZLE    # tangential width of tongue — QUALITY-TIER 1.6×1.6
KEY_DEPTH  = 2 * NOZZLE    # radial protrusion of tongue (was 2.0×1.0, its 1.0 rib
                    # under the two-bead tier, user-caught; same treatment
                    # as the bearing-cap tenons — hosts whose wall around
                    # the deeper groove would drop under 1.6 grow local
                    # BOSSES, see chambers._cap)
...
# Imported at the top of this module from the SHARED cadkit/fasteners.py
# (M2_SELFTAP_D, M2_SHAFT_CLR_D, M2_HEAD_RECESS_D/H, M2_INSERT_PILOT_D,
# M2_INSERT_DEPTH). The design is hardware-free now — any future M2 feature
# references these, never a local copy.
