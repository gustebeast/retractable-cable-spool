...
PRINT-DIRECTION story (user redesign — the library now KNOWS direction,
not just axis): in the joint's frame (tenons standing, mount.py flips the
solids to the hanging world pose) the bracket builds 'up' (bar on the
bed, tenons rising: stem → 45° flare → plain flat last layer) and
frame_top builds 'down' (it prints world-upright, which is INVERTED
relative to the flipped joint frame): the cavity's wide flat end prints
FIRST as a fully-supported floor, then the 45° flares narrow the void
layer by layer. joint() dispatches that up+down site to the flat-top
MUSHROOM — the dull arrowhead with its tip fully flattened: no tapers,
no one-nozzle roof cap, shorter swallow, more flat bearing contact.
"""
...
from .params import (
    ...
    MOUNT_CROSS_HALF, MOUNT_SCREW_A_X, MOUNT_SCREW_B_X, MOUNT_R_OUT,
...
_NOZ = 0.8
_UP   = PrintSpec(nozzle=_NOZ, facing="up")     # bracket: bar on bed, tenons rise
_DOWN = PrintSpec(nozzle=_NOZ, facing="down")   # frame_top: builds toward the
                                                # cavity opening (inverted in the
                                                # joint frame) -> flat-top profile
_MOUNT_J = joint(MOUNT_TEN_W, None, tenon=_UP, mortise=_DOWN,
                 install="-x", clearance=MOUNT_CLR)  # bracket seats sliding −x
                                                    # (stops at the mortises'
                                                    # −x ends... the tenon
                                                    # travels −x to its stop
...
def mount_channel_cuts(channel_angles=(0.0, 90.0),
                       groove_angles=(0.0, 90.0, 180.0, 270.0)):
    """The four channel cutters (X pattern, rotated to each angle in
    `channel_angles`) + the LOCK grooves at `groove_angles`, in world
    position — frame.py subtracts the full set from frame_top; the
    frame-joint coupon takes the X axis only. Mortise segments retain;
    pockets are the z-entry, open at the rib's top face. Each
    mortise+pocket pair shares a boundary plane, cutting one continuous
    channel."""
    cuts = []
    for x0, ln, pocket in ((MOUNT_MORT_A_X0, MOUNT_MORT_L, False),
        ...
        c = (_flip(_MOUNT_J.mortise(drop=2.0, pocket=pocket, length=ln))
             .translate((x0, 0.0, TOP_RIB_Z1)))
        for a in channel_angles:
            cuts.append(c.rotate((0, 0, 0), (0, 0, 1), a))
    ...
    g = (cq.Workplane("XY")
         .box(MOUNT_LOCK_W, MOUNT_LOCK_L + 2.0, MOUNT_LOCK_D + 0.5,
...
def _build_bracket(strip=False):
    ...
    flip implies: +Z→−Z, support-free. strip=True builds the JOINT-TEST
    variant (user's spec): the tenon bar alone — no cross arm, no lock
    grooves — for the frame-joint coupon's one-axis channel test."""
    t = MOUNT_PLATE_T
    bar = (cq.Workplane("XY")
    ...
    if not strip:
        # PLUS: the joinery-free cross arm (lateral support — rides the
        # other ribs' top faces; no tenons, no screw holes on this axis)
        bar = bar.union(
            cq.Workplane("XY")
            .box(MOUNT_PLATE_W, 2.0 * MOUNT_CROSS_HALF, t,
                 centered=(True, True, False)))
        # lock grooves in the cross arm's world-UNDERSIDE (= print-top
        # face, z=t here), BOTH ends — keeps the part y-symmetric, so the
        # build→world flip can't move the lock to the wrong end
        for sy in (+1.0, -1.0):
            y0 = sy * MOUNT_LOCK_Y0
            bar = bar.cut(
                cq.Workplane("XY")
    ...
    for x0 in (MOUNT_TEN_A_X0, MOUNT_TEN_B_X0):
        bar = bar.union(
            _MOUNT_J.tenon(root=1.0, length=MOUNT_TEN_L)
            .translate((x0, 0.0, t)))
    for sx in (MOUNT_SCREW_A_X, MOUNT_SCREW_B_X):
...
test_mount_strip = _build_bracket(strip=True)
